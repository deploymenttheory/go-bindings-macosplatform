...
var (
	...
	// Allocates an AEDesc (given a Null Desc) constructed from a flattened data buffer produced by calling AEFlattenDesc. - Parameters: - buffer: A pointer to data produced by `AEFlattenDesc` - bufferLen: The size of the data referenced by `buffer` - result: On successful completion, a pointer to an `AEDesc*` containing the unflattened descriptor. The caller is responsible for disposing of it. - Returns: `noErr` on success, `paramErr` if the buffer could not be parsed, or `memFullErr` for irrational memory sizes.
