...
const (
	...
	// An error caused by a failed attempt to use the App Attest key. You receive this error if something goes wrong with generating, retrieving, or using an App Attest cryptographic key, when: - You call ``DeviceCheck/DCAppAttestService/attestKey:clientDataHash:completionHandler:`` for a key that’s already been attested. - You call ``DeviceCheck/DCAppAttestService/generateAssertion:clientDataHash:completionHandler:`` with an unattested key. - The App Attest service cannot enforce the key access control policy on macOS. - The App Attest service rejects the key.
	DCErrorInvalidKey DCError = 3
	// An error that indicates a failed attempt to contact the App Attest service during an attestation. You receive this error when you call ``DeviceCheck/DCAppAttestService/attestKey:clientDataHash:completionHandler:`` and the framework isn’t able to complete the attestation. If you receive this error, try the attestation again later using the same key and the same value for the `clientDataHash` parameter. Retrying with the same inputs helps to preserve the risk metric for a given device.
...
func (e DCError) String() string {
	...
	default:
		return fmt.Sprintf("DCError(%d)", int64(e))
	}
}
