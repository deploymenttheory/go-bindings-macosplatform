...
// Bitmask — values may be combined with |.
type HostObjectDataOptions uint64

const (
	HostObjectDataOptionsNone             HostObjectDataOptions = 0
	HostObjectDataOptionsKernelUserShared HostObjectDataOptions = 1
)

// String returns the HostObjectDataOptions constant's name, or its numeric form when the
// value is not a known constant.
func (e HostObjectDataOptions) String() string {
	var parts []string
	if e&HostObjectDataOptionsKernelUserShared != 0 {
		parts = append(parts, "HostObjectDataOptionsKernelUserShared")
	}
	if len(parts) == 0 {
		return "0"
	}
