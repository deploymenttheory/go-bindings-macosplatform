...
// Bitmask — values may be combined with |.
type MediaSourceType uint64

const (
	// Audio source type. Includes iTunes, GarageBand, and Logic.
	MediaSourceTypeAudio MediaSourceType = 1
	// Image source type. Includes iPhoto, Aperture, and Photo Booth.
	MediaSourceTypeImage MediaSourceType = 2
	// Movie source type. Includes iMovie and Final Cut Pro.
	MediaSourceTypeMovie MediaSourceType = 4
)

// String returns the MediaSourceType constant's name, or its numeric form when the
// value is not a known constant.
func (e MediaSourceType) String() string {
	var parts []string
	if e&MediaSourceTypeAudio != 0 {
		parts = append(parts, "MediaSourceTypeAudio")
	}
	if e&MediaSourceTypeImage != 0 {
		parts = append(parts, "MediaSourceTypeImage")
	}
	if e&MediaSourceTypeMovie != 0 {
		parts = append(parts, "MediaSourceTypeMovie")
	}
	if len(parts) == 0 {
		return "0"
	}
