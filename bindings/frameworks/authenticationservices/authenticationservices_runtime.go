...
func SymbolAvailable(symbol string) bool {
	...
	return _authenticationservicesLib != 0 && !_failedSymbols[symbol]
}

func _loadLibrary() {
	var err error
	_authenticationservicesLib, err = purego.Dlopen("/System/Library/Frameworks/AuthenticationServices.framework/AuthenticationServices", purego.RTLD_GLOBAL|purego.RTLD_LAZY)
	if err != nil {
		if os.Getenv("PUREGO_BINDINGS_DEBUG") != "" {
			fmt.Fprintf(os.Stderr, "purego-bindings: %s unavailable (%v) — C functions will be no-ops\n", "AuthenticationServices", err)
		}
		return
	}
