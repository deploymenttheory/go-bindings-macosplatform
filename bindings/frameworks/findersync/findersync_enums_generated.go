...
type MenuKind uint64

const (
	// A shortcut menu created when the user control-clicks on an item or a group of selected items inside the Finder window.
	MenuKindContextualMenuForItems MenuKind = 0
	// A shortcut menu created when the user control-clicks on the Finder window’s background.
	MenuKindContextualMenuForContainer MenuKind = 1
	// A shortcut menu created when the user control-clicks on an item in the sidebar.
	MenuKindContextualMenuForSidebar MenuKind = 2
	// A menu created when the user clicks on the extension’s toolbar button.
	MenuKindToolbarItemMenu MenuKind = 3
)

// String returns the MenuKind constant's name, or its numeric form when the
// value is not a known constant.
func (e MenuKind) String() string {
	switch e {
	case MenuKindContextualMenuForItems:
		return "MenuKindContextualMenuForItems"
	case MenuKindContextualMenuForContainer:
		return "MenuKindContextualMenuForContainer"
	case MenuKindContextualMenuForSidebar:
		return "MenuKindContextualMenuForSidebar"
	case MenuKindToolbarItemMenu:
		return "MenuKindToolbarItemMenu"
	default:
		return fmt.Sprintf("MenuKind(%d)", int64(e))
	}
}
