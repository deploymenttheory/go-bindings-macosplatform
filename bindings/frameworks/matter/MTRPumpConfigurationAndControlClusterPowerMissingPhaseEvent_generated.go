// Code generated by go-bindings-codegen. DO NOT EDIT.

//go:build darwin

package matter

import (
	"runtime"

	"github.com/deploymenttheory/go-bindings-macosplatform/bindings/internal/objref"
	"github.com/deploymenttheory/go-bindings-macosplatform/bindings/runtime/obj"
	"github.com/deploymenttheory/go-bindings-macosplatform/bindings/runtime/purego"
	"github.com/deploymenttheory/go-bindings-macosplatform/bindings/runtime/rt"
	"github.com/ebitengine/purego/objc"
)

// MTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent is an idiomatic wrapper over the Objective-C class MTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent.
type MTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent struct {
	objref.Handle
}

// MTRPumpConfigurationAndControlClusterPowerMissingPhaseEventFromID adopts an existing Objective-C object as a MTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent
// (nil for 0), retaining it and registering a release finalizer.
func MTRPumpConfigurationAndControlClusterPowerMissingPhaseEventFromID(id objc.ID) *MTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent {
	if id == 0 {
		return nil
	}
	x := &MTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent{}
	x.Handle = objref.Wrap(purego.Retain(id))
	objref.Track(x)
	return x
}

// mTRPumpConfigurationAndControlClusterPowerMissingPhaseEventAdopt wraps an Objective-C object that this code just created as a
// MTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent (nil for 0). The caller already owns the object's reference,
// so this does not add another; it only arranges for the object to be released
// once Go stops using it. Constructors use it.
func mTRPumpConfigurationAndControlClusterPowerMissingPhaseEventAdopt(id objc.ID) *MTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent {
	if id == 0 {
		return nil
	}
	x := &MTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent{}
	x.Handle = objref.Wrap(id)
	objref.Track(x)
	return x
}

// Description returns the object's -description text.
func (mpcaccpmpe *MTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent) Description() string {
	defer runtime.KeepAlive(mpcaccpmpe)
	return rt.Description(objref.IDOf(mpcaccpmpe))
}

// IsEqual reports Objective-C equality (isEqual:) with another object.
func (mpcaccpmpe *MTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent) IsEqual(other obj.Object) bool {
	defer runtime.KeepAlive(mpcaccpmpe)
	defer runtime.KeepAlive(other)
	return rt.IsEqual(objref.IDOf(mpcaccpmpe), objref.IDOf(other))
}

// IsKind reports whether the object is an instance of the named class or a subclass.
func (mpcaccpmpe *MTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent) IsKind(className string) bool {
	defer runtime.KeepAlive(mpcaccpmpe)
	return rt.IsKind(objref.IDOf(mpcaccpmpe), className)
}

// String returns the object's -description text, so a wrapper prints usefully
// under fmt.
func (mpcaccpmpe *MTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent) String() string {
	defer runtime.KeepAlive(mpcaccpmpe)
	return rt.Description(objref.IDOf(mpcaccpmpe))
}

// NewMTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent creates a new MTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent.
func NewMTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent() *MTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent {
	_id := objc.Send[objc.ID](objc.ID(_class("MTRPumpConfigurationAndControlClusterPowerMissingPhaseEvent")), objc.RegisterName("new"))
	return mTRPumpConfigurationAndControlClusterPowerMissingPhaseEventAdopt(_id)
}
