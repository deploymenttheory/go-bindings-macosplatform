...
The current bindings require **macOS 27+**. Frameworks and C libraries use arm64 metadata from **Xcode 27.0 / macOS SDK 27.0** and both use purego. See [the extraction workflow](docs/extraction_workflow.md) for the current SDK upgrade procedure and verification gates.
...
### Emitters (`internal/codegen/emit/raw/libraries/`)
...
### Emit tree — style-first, four parallel leaves (`internal/codegen/emit/`)

All four code-emitters live under a single **style-first** tree; the raw-vs-idiomatic
boundary is the top-level seam and each style emits both pipelines. The per-pipeline
front-ends (scanner/`meta`/`typemap`/`naming`/`pipeline`) stay under
`internal/codegen/frameworks/` and `internal/codegen/libraries/`.
...
`<leaf>/*.go` (gather: resolves metadata + types into the IR; the only place type
decisions live) · `<leaf>/view/` (pkg `view`: pure-data IR structs, no meta/typemap
imports) · `<leaf>/render/` (pkg `render`: `Execute`/typed render funcs + `FuncMap`
+ `//go:embed templates/*.tmpl`; imports only `view` — the render firewall) ·
`<leaf>/render/templates/*.tmpl`. Leaf packages get distinctive names
(`rawfw`/`rawlib`/`idiofw`/`idiolib`) so importers need no aliases.
...
stubs. Shared file scaffold for the frameworks leaves:
`internal/codegen/shared/fileasm` — `Assemble(File)` + `file.tmpl` + the two import
groupers (`ImportLinesStdlibMod`, `ImportLinesStdlibExternalInternal`).
...
The runtime layer lives under `bindings/runtime/` — **public** packages imported both by the generated code and by external consumers (so consuming projects only ever import `bindings/…`). Both pipelines are now pure Go; there is no CGo runtime:
...
- **`bindings/runtime/objptr`** (package `objptr`) — a dependency-free `interface{ Ptr() unsafe.Pointer }`. It is the common object constraint referenced by generated library protocol interfaces, so a `CGO_ENABLED=0` consumer of `bindings/libraries/…` pulls in nothing else.

The C-library packages (`bindings/libraries/`) are generated against the purego backend: each `<pkg>_runtime_generated.go` `dlopen`s the library and registers its symbols, and calls dispatch through `purego.SyscallN` with no C compiler, no bridge `.m` files, and no `import "C"`. Generated library functions and methods take **no** `context.Context` parameter and have **no** telemetry — the same zero-overhead, uninstrumented dispatch as the purego framework packages. Main-thread dispatch for AppKit-adjacent calls lives in `opinionated/tools/grandcentraldispatch/mainthread` (`Do`, `PumpMainRunLoop`).
...
| **Template structs** | `*Model` suffix for all structs in `emit/raw/libraries/view/model.go` |
