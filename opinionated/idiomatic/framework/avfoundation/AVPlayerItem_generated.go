...
func PlayerItemFromID(id objc.ID) *PlayerItem {
	...
	x := &PlayerItem{Handle: objref.Wrap(purego.Retain(id))}
	objref.Track(x)
	return x
}
...
func playerItemAdopt(id objc.ID) *PlayerItem {
	...
	x := &PlayerItem{Handle: objref.Wrap(id)}
	objref.Track(x)
	return x
}
...
//
// WithAutomaticallyPreservesTimeOffsetFromLive sets automaticallyPreservesTimeOffsetFromLive and returns the receiver so calls can be chained.
...
//
// WithSeekingWaitsForVideoCompositionRendering sets seekingWaitsForVideoCompositionRendering and returns the receiver so calls can be chained.
...
//
// WithTextStyleRules sets the collection and returns the receiver so calls can be chained.
...
//
// WithVideoApertureMode sets videoApertureMode and returns the receiver so calls can be chained.
...
//
// WithAppliesPerFrameHDRDisplayMetadata sets appliesPerFrameHDRDisplayMetadata and returns the receiver so calls can be chained.
...
//
// WithAudioSpatializationAllowed sets audioSpatializationAllowed and returns the receiver so calls can be chained.
...
//
// WithCanUseNetworkResourcesForLiveStreamingWhilePaused sets canUseNetworkResourcesForLiveStreamingWhilePaused and returns the receiver so calls can be chained.
...
//
// WithPreferredForwardBufferDuration sets preferredForwardBufferDuration and returns the receiver so calls can be chained.
...
//
// WithPreferredPeakBitRate sets preferredPeakBitRate and returns the receiver so calls can be chained.
...
//
// WithPreferredPeakBitRateForExpensiveNetworks sets preferredPeakBitRateForExpensiveNetworks and returns the receiver so calls can be chained.
...
//
// WithStartsOnFirstEligibleVariant sets startsOnFirstEligibleVariant and returns the receiver so calls can be chained.
...
//
// WithVariantPreferences sets variantPreferences and returns the receiver so calls can be chained.
...
//
// WithPreferredCustomMediaSelectionSchemes sets the collection and returns the receiver so calls can be chained.
...
//
// WithAutomaticallyHandlesInterstitialEvents sets automaticallyHandlesInterstitialEvents and returns the receiver so calls can be chained.
