...
func CapturePhotoSettingsFromID(id objc.ID) *CapturePhotoSettings {
	...
	x := &CapturePhotoSettings{Handle: objref.Wrap(purego.Retain(id))}
	objref.Track(x)
	return x
}
...
func capturePhotoSettingsAdopt(id objc.ID) *CapturePhotoSettings {
	...
	x := &CapturePhotoSettings{Handle: objref.Wrap(id)}
	objref.Track(x)
	return x
}
...
//
// WithPhotoQualityPrioritization sets photoQualityPrioritization and returns the receiver so calls can be chained.
...
//
// WithHighResolutionPhotoEnabled sets highResolutionPhotoEnabled and returns the receiver so calls can be chained.
...
//
// WithConstantColorFallbackPhotoDeliveryEnabled sets constantColorFallbackPhotoDeliveryEnabled and returns the receiver so calls can be chained.
...
//
// WithShutterSoundSuppressionEnabled sets shutterSoundSuppressionEnabled and returns the receiver so calls can be chained.
...
// Specifies whether the photo will be captured with constant color. Default is NO. Set to YES if you wish to capture a constant color photo. Throws an exception if -[AVCapturePhotoOutput constantColorEnabled] is not set to YES.
...
// Specifies whether a fallback photo is delivered when taking a constant color capture. Default is NO. Set to YES if you wish to receive a fallback photo that can be used in case the main constant color photo's confidence level is too low for your use case.
...
// Specifies whether the built-in shutter sound should be suppressed when capturing a photo with these settings. Default is NO. Set to YES if you wish to suppress AVCapturePhotoOutput's built-in shutter sound for this request. AVCapturePhotoOutput throws an NSInvalidArgumentException in `-capturePhotoWithSettings:` if its `shutterSoundSuppressionSupported` property returns NO.
