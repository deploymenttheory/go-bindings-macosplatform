// Code generated by go-bindings-codegen. DO NOT EDIT.

//go:build darwin

package metalperformanceshaders

import (
	"github.com/deploymenttheory/go-bindings-macosplatform/bindings/runtime/purego"
	"github.com/deploymenttheory/go-bindings-macosplatform/opinionated/idiomatic/framework/metal"
	"github.com/deploymenttheory/go-bindings-macosplatform/opinionated/idiomatic/framework/mpscore"
	"github.com/deploymenttheory/go-bindings-macosplatform/opinionated/idiomatic/internal/objref"
	"github.com/ebitengine/purego/objc"
)

// ImageThresholdBinaryInverse is an idiomatic wrapper over the Objective-C class MPSImageThresholdBinaryInverse.
//
// It embeds [UnaryImageKernel], promoting that type's methods.
//
// A filter that returns 0 for each pixel with a value greater than a specified threshold or a specified value otherwise.
type ImageThresholdBinaryInverse struct {
	UnaryImageKernel
}

// ImageThresholdBinaryInverseFromID adopts an existing Objective-C object as a ImageThresholdBinaryInverse
// (nil for 0), retaining it and registering a release finalizer.
func ImageThresholdBinaryInverseFromID(id objc.ID) *ImageThresholdBinaryInverse {
	if id == 0 {
		return nil
	}
	x := &ImageThresholdBinaryInverse{}
	x.Handle = objref.Wrap(purego.Retain(id))
	objref.Track(x)
	return x
}

// imageThresholdBinaryInverseAdopt wraps an Objective-C object that this code just created as a
// ImageThresholdBinaryInverse (nil for 0). The caller already owns the object's reference,
// so this does not add another; it only arranges for the object to be released
// once Go stops using it. Constructors use it.
func imageThresholdBinaryInverseAdopt(id objc.ID) *ImageThresholdBinaryInverse {
	if id == 0 {
		return nil
	}
	x := &ImageThresholdBinaryInverse{}
	x.Handle = objref.Wrap(id)
	objref.Track(x)
	return x
}

// NewImageThresholdBinaryInverse creates a new ImageThresholdBinaryInverse.
func NewImageThresholdBinaryInverse() *ImageThresholdBinaryInverse {
	_id := objc.Send[objc.ID](objc.ID(_class("MPSImageThresholdBinaryInverse")), objc.RegisterName("new"))
	return imageThresholdBinaryInverseAdopt(_id)
}

// WithOffset sets the position of the destination clip rectangle origin relative to the source buffer.
func (itbi *ImageThresholdBinaryInverse) WithOffset(offset mpscore.MPSOffset) *ImageThresholdBinaryInverse {
	objc.Send[objc.ID](objref.IDOf(itbi), objc.RegisterName("setOffset:"), offset)
	return itbi
}

// WithClipRect sets an optional clip rectangle to use when writing data. Only the pixels in the rectangle will be overwritten.
func (itbi *ImageThresholdBinaryInverse) WithClipRect(clipRect metal.MTLRegion) *ImageThresholdBinaryInverse {
	objc.Send[objc.ID](objref.IDOf(itbi), objc.RegisterName("setClipRect:"), clipRect)
	return itbi
}

// WithLabel sets the string that identifies the kernel.
func (itbi *ImageThresholdBinaryInverse) WithLabel(label string) *ImageThresholdBinaryInverse {
	objc.Send[objc.ID](objref.IDOf(itbi), objc.RegisterName("setLabel:"), purego.NSString(label))
	return itbi
}

// ThresholdValue returns the threshold value used to init the threshold filter
func (itbi *ImageThresholdBinaryInverse) ThresholdValue() float32 {
	_r := objc.Send[float32](objref.IDOf(itbi), objc.RegisterName("thresholdValue"))
	return _r
}

// MaximumValue returns the maximum value used to init the threshold filter
func (itbi *ImageThresholdBinaryInverse) MaximumValue() float32 {
	_r := objc.Send[float32](objref.IDOf(itbi), objc.RegisterName("maximumValue"))
	return _r
}

var _ UnaryImageKernelProvider = (*ImageThresholdBinaryInverse)(nil)

var _ KernelProvider = (*ImageThresholdBinaryInverse)(nil)
