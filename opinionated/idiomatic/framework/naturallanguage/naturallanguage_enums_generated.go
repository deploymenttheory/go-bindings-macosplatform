...
// Bitmask — values may be combined with |.
type TokenizerAttributes uint64

const (
	TokenizerAttributeNumeric  TokenizerAttributes = 1
	TokenizerAttributeSymbolic TokenizerAttributes = 2
	TokenizerAttributeEmoji    TokenizerAttributes = 4
)

// String returns the TokenizerAttributes constant's name, or its numeric form when the
// value is not a known constant.
func (e TokenizerAttributes) String() string {
	var parts []string
	if e&TokenizerAttributeNumeric != 0 {
		parts = append(parts, "TokenizerAttributeNumeric")
	}
	if e&TokenizerAttributeSymbolic != 0 {
		parts = append(parts, "TokenizerAttributeSymbolic")
	}
	if e&TokenizerAttributeEmoji != 0 {
		parts = append(parts, "TokenizerAttributeEmoji")
	}
	if len(parts) == 0 {
		return "0"
	}
