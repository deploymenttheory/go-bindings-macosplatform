...
func VirtioConsolePortConfigurationArrayFromID(id objc.ID) *VirtioConsolePortConfigurationArray {
	...
	x := &VirtioConsolePortConfigurationArray{Handle: objref.Wrap(purego.Retain(id))}
	objref.Track(x)
	return x
}
...
func virtioConsolePortConfigurationArrayAdopt(id objc.ID) *VirtioConsolePortConfigurationArray {
	...
	x := &VirtioConsolePortConfigurationArray{Handle: objref.Wrap(id)}
	objref.Track(x)
	return x
}
...
//
// WithMaximumPortCount sets maximumPortCount and returns the receiver so calls can be chained.
