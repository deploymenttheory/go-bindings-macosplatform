// Code generated by go-bindings-codegen. DO NOT EDIT.

//go:build darwin

package foundation

import (
	"unsafe"

	"github.com/deploymenttheory/go-bindings-macosplatform/bindings/runtime/purego"
	"github.com/deploymenttheory/go-bindings-macosplatform/opinionated/idiomatic/internal/objref"
	"github.com/deploymenttheory/go-bindings-macosplatform/opinionated/idiomatic/obj"
	"github.com/deploymenttheory/go-bindings-macosplatform/opinionated/idiomatic/rt"
	"github.com/ebitengine/purego/objc"
)

// UnitArea is an idiomatic wrapper over the Objective-C class NSUnitArea.
//
// It embeds [Dimension], promoting that type's methods.
//
// A unit of measure for area.
type UnitArea struct {
	Dimension
}

// UnitAreaFromID adopts an existing Objective-C object as a UnitArea
// (nil for 0), retaining it and registering a release finalizer.
func UnitAreaFromID(id objc.ID) *UnitArea {
	if id == 0 {
		return nil
	}
	x := &UnitArea{}
	x.Handle = objref.Wrap(purego.Retain(id))
	objref.Track(x)
	return x
}

// unitAreaAdopt wraps an Objective-C object that this code just created as a
// UnitArea (nil for 0). The caller already owns the object's reference,
// so this does not add another; it only arranges for the object to be released
// once Go stops using it. Constructors use it.
func unitAreaAdopt(id objc.ID) *UnitArea {
	if id == 0 {
		return nil
	}
	x := &UnitArea{}
	x.Handle = objref.Wrap(id)
	objref.Track(x)
	return x
}

// NewUnitArea creates a new UnitArea.
func NewUnitArea() *UnitArea {
	_id := objc.Send[objc.ID](objc.ID(_class("NSUnitArea")), objc.RegisterName("new"))
	return unitAreaAdopt(_id)
}

// WithObservationInfo sets the observation info.
func (ua *UnitArea) WithObservationInfo(observationInfo unsafe.Pointer) *UnitArea {
	objc.Send[objc.ID](objref.IDOf(ua), objc.RegisterName("setObservationInfo:"), observationInfo)
	return ua
}

// WithScriptingProperties sets the scripting properties.
func (ua *UnitArea) WithScriptingProperties(scriptingProperties map[string]obj.Object) *UnitArea {
	objc.Send[objc.ID](objref.IDOf(ua), objc.RegisterName("setScriptingProperties:"), rt.MapToDict(scriptingProperties, func(_k string) objc.ID { return purego.NSString(_k) }, func(_v obj.Object) objc.ID { return objref.IDOf(_v) }))
	return ua
}

var _ DimensionProvider = (*UnitArea)(nil)

var _ UnitProvider = (*UnitArea)(nil)
