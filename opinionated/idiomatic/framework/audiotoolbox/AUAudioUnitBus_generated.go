...
func AudioUnitBusFromID(id objc.ID) *AudioUnitBus {
	...
	x := &AudioUnitBus{Handle: objref.Wrap(purego.Retain(id))}
	objref.Track(x)
	return x
}
...
func audioUnitBusAdopt(id objc.ID) *AudioUnitBus {
	...
	x := &AudioUnitBus{Handle: objref.Wrap(id)}
	objref.Track(x)
	return x
}
...
func NewAudioUnitBusWithFormatError(format obj.Object) (*AudioUnitBus, error) {
...
//
// WithContextPresentationLatency sets contextPresentationLatency and returns the receiver so calls can be chained.
...
//
// WithSupportedChannelCounts sets the collection and returns the receiver so calls can be chained.
...
//
// WithMaximumChannelCount sets maximumChannelCount and returns the receiver so calls can be chained.
...
// The audio format and channel layout of audio being transferred on the bus. Bridged to the v2 property kAudioUnitProperty_StreamFormat.
...
// Whether the bus is active. Hosts must enable input busses before using them. The reason for this is to allow a unit such as a mixer to be prepared to render a large number of inputs, but avoid the work of preparing to pull inputs which are not in use. Bridged to the v2 properties kAudioUnitProperty_MakeConnection and kAudioUnitProperty_SetRenderCallback.
...
// An array of numbers giving the supported numbers of channels for this bus. If supportedChannelCounts is nil, then any number less than or equal to maximumChannelCount is supported. If setting supportedChannelCounts makes the current format unsupported, then format will be set to nil. The default value is nil.
//
...
// The maximum numbers of channels supported for this bus. If supportedChannelCounts is set, then this value is derived from supportedChannelCounts. If setting maximumChannelCount makes the current format unsupported, then format will be set to nil. The default value is UINT_MAX.
