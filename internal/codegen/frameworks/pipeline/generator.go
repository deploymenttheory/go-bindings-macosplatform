...
type BindingsConfig struct {
	...
	// to the metas it accepts. Used by the cgo→purego migration ratchet: only
	// libraries with backend "purego" are emitted here, the rest stay on the
	// CGo pipeline. When set, LibrariesOutDir is NOT wiped wholesale — each
	// accepted library's package dir is replaced individually, so the two
	// pipelines can share one output tree.
...
func GenerateBindings(cfg BindingsConfig) error {
	...
	if cfg.LibrariesOutDir != "" && cfg.LibraryFilter == nil {
	...
	for _, m := range ordered {
		...
		if isLibrary && cfg.LibraryFilter != nil && !cfg.LibraryFilter(m.Framework) {
			continue
		}
...
func generateFramework(m *meta.FrameworkMeta, outBase string, cfg BindingsConfig) error {
	...
	// With a LibraryFilter the shared libraries tree is not wiped wholesale, so
	// a library migrating from the CGo pipeline still has its old package files
	// (cgo.go, bridge/, *_bridge_impl.m) on disk — replace the package dir.
	if m.LinkLib != "" && cfg.LibraryFilter != nil {
...
func writeRuntimeGo(
	...
	fmt.Fprintf(&buf, "//go:build darwin\n\n")
	fmt.Fprintf(&buf, "package %s\n\n", packageName)
	// A C library has no ObjC classes, so its runtime file never needs the objc
	// package (no _objcClass helper is emitted below), and its dylib load
	// failure is always tolerated (the fmt+os debug path), never fatal.
	tolerantLoad := len(regLines) > 0 || isLibrary
