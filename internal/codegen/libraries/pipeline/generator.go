...
func emitFramework(
	...
	// Purego backend (clibraries.json "backend": "purego"): identical Go
	// surface, but function/extern bodies bind dlopen'd symbols through purego
	// instead of the generated ObjC bridge — no cgo.go, no bridge/ directory.
	isPurego := isLibrary && scanner.CLibraryBackend(framework.Framework) == scanner.BackendPurego
	...
	// Purego backend: <pkg>_runtime.go replaces the bridge — Dlopen + tolerant
	// per-symbol registration + extern population.
	if isPurego {
