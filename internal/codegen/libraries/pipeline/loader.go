...
func filterInheritedClassMethods(reg *Registry) {
	...
	ownerFramework := make(map[string]*macosplatformmetadata.FrameworkMeta, len(reg.OwnerIndex))
	for _, framework := range reg.Frameworks {
	...
	for className := range reg.ClassIndex {
		...
		novel := make([]macosplatformmetadata.Method, 0, len(class.Methods))
...
func classMethodIsBridgeable(method macosplatformmetadata.Method) bool {
...
func validateSuperchainAcyclic(allClasses map[string]macosplatformmetadata.Class) error {
...
func indexProtocolProxiesFromMethods(
	extensions map[string][]macosplatformmetadata.Method,
	knownProtocols, dst map[string]string,
) {
