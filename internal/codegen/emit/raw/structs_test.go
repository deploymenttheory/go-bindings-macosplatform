...
func newStructFM(structs map[string]meta.Struct, typedefs map[string]string) *meta.FrameworkMeta {
...
func TestStructsEmpty(t *testing.T) {
	...
	framework := newStructFM(map[string]meta.Struct{}, map[string]string{})
	m := newStructMapper()
...
func TestStructBasicFields(t *testing.T) {
	var buf bytes.Buffer
	framework := newStructFM(map[string]meta.Struct{
		"NSPoint": {
			Fields: []meta.StructField{
...
func TestStructAvailabilityComment(t *testing.T) {
	...
	framework := newStructFM(map[string]meta.Struct{
		"NSDirectionalEdgeInsets": {
			Availability: meta.Availability{MacOSIntroduced: "10.15"},
			Fields: []meta.StructField{
...
func TestTypedefAlias(t *testing.T) {
	...
	framework := newStructFM(
		map[string]meta.Struct{
			"_NSRange": {
				Fields: []meta.StructField{{Name: "location", GoType: "uint64"}},
			},
		},
		map[string]string{
...
func TestTypedefAliasSkipsSameName(t *testing.T) {
	...
	framework := newStructFM(
		map[string]meta.Struct{
			"NSRect": {
				Fields: []meta.StructField{{Name: "origin", GoType: "unsafe.Pointer"}},
			},
		},
		map[string]string{
...
func TestTypedefAliasSkipsNonStruct(t *testing.T) {
	...
	framework := newStructFM(
		map[string]meta.Struct{
			"SomeStruct": {
				Fields: []meta.StructField{{Name: "val", GoType: "int32"}},
			},
		},
		map[string]string{
...
func TestStructSortOrder(t *testing.T) {
	var buf bytes.Buffer
	framework := newStructFM(map[string]meta.Struct{
		"ZPoint": {Fields: []meta.StructField{{Name: "z", GoType: "float64"}}},
		"APoint": {Fields: []meta.StructField{{Name: "a", GoType: "float64"}}},
		"MPoint": {Fields: []meta.StructField{{Name: "m", GoType: "float64"}}},
	}, nil)
	m := newStructMapper()
	if _, err := EmitStructs(&buf, framework, m, nil); err != nil {
		t.Fatalf("unexpected error: %v", err)
