...
func EmitProtocolImpls(outDir string, framework *meta.FrameworkMeta, m *typemap.Mapper) error {
...
func collectProtocolMethods(proto meta.Protocol, m *typemap.Mapper) []overrideMethod {
...
func buildProtocolImplGoFileModel(packageName, framework, protoName string, methods []overrideMethod, m *typemap.Mapper) protocolImplGoFileModel {
	...
	if hasProxy {
		...
		fmt.Fprintf(&factoryComment, "// The returned objc.Object wraps the +1-retained ObjC delegate and\n")
	...
	if hasProxy {
		...
		fmt.Fprintf(&returnBody, "\twrapped := objc.WrapObject(obj)\n")
		fmt.Fprintf(&returnBody, "\tif wrapped != nil {\n")
		fmt.Fprintf(&returnBody, "\t\tobjc.Track(wrapped, wrapped.Ptr)\n")
