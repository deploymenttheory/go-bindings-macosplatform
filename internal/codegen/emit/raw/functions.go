...
func EmitFunctions(w io.Writer, pkgName, packageName string, framework *meta.FrameworkMeta, m *typemap.Mapper, knownClasses map[string]bool) error {
...
func buildFunctionsModel(pkgName, packageName string, framework *meta.FrameworkMeta, m *typemap.Mapper, knownClasses map[string]bool) functionsFileModel {
...
func buildFunctionModel(fn meta.Function, framework, _ string, ctx typemap.Context, m *typemap.Mapper, imports typemap.ImportSet) functionModel {
...
func buildFunctionCallBody(callExpr, retType string, m *typemap.Mapper) string {
	...
	case retType == "objc.Object":
		fmt.Fprintf(&sb, "\t_ptr := unsafe.Pointer(%s)\n", callExpr)
		fmt.Fprintf(&sb, "\ttel.RaiseIfException(ctx, _exc)\n")
		fmt.Fprintf(&sb, "\treturn objc.WrapObject(_ptr)\n")
	...
	case isValueStructReturn(retType, m):
		...
		fmt.Fprintf(&sb, "\tobjc.FreePtr(_ptr)\n")
...
func buildFunctionsImports(functions []functionModel, usedImports typemap.ImportSet) []string {
	// Scan call bodies, preambles, and signatures (return types can reference objc.Object).
	...
	if len(functions) > 0 {
		set["context"] = true
		set["github.com/deploymenttheory/go-bindings-macosplatform/internal/tel"] = true
		if bytes.Contains(body, []byte("blocks.MakeBlock_")) {
			set["github.com/deploymenttheory/go-bindings-macosplatform/internal/blocks"] = true
		}
		if bytes.Contains(body, []byte("objc.")) {
...
func writeFunction(w io.Writer, fn meta.Function, framework string, ctx typemap.Context, m *typemap.Mapper) error {
	...
	return executeTemplate(w, "functions_file", functionsFileModel{
		...
		FwLower:   packageName,
		Imports:   []string{"context", "unsafe", "github.com/deploymenttheory/go-bindings-macosplatform/internal/tel"},
		Functions: []functionModel{model},
	})
}

func hasVAListArgFn(fn meta.Function) bool {
