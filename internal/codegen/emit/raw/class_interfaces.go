...
func EmitClassInterfaces(w io.Writer, pkgName string, framework *meta.FrameworkMeta, m *typemap.Mapper, knownClasses map[string]bool, allClasses map[string]meta.Class) error {
	...
	for _, iface := range interfaces {
		...
		if strings.Contains(iface.EmbedLine, "objc.") {
			usesObjc = true
		}
		for _, method := range iface.Methods {
			...
			if strings.Contains(method.Params, "objc.") || strings.Contains(method.Ret, "objc.") {
				usesObjc = true
			}
		}
	}
	...
	if usesObjc {
		allImports = append(allImports, "github.com/deploymenttheory/go-bindings-macosplatform/internal/objc")
	}
...
func buildClassInterfaceModel(name string, cls meta.Class, framework *meta.FrameworkMeta, m *typemap.Mapper, ctx typemap.Context, allClasses map[string]meta.Class, usedImports typemap.ImportSet) classInterfaceModel {
