...
func TestCollectProtocolMethodsIMPSafe(t *testing.T) {
	...
	proto := meta.Protocol{
		Methods: []meta.Method{
			...
			{Selector: "initWith:", IsInit: true, Return: meta.ReturnType{IsInstancetype: true}}, // excluded
			{Selector: "bar:", IsClassMethod: true, Return: meta.ReturnType{ObjCType: "void"}},   // excluded
		},
	}
	result := collectProtocolMethods(proto, m)
	if len(result) != 1 {
...
func TestEmitProtocolImplGoWithoutProxy(t *testing.T) {
	...
	if !strings.Contains(out, "objc.Object") {
		t.Errorf("without proxy, return type should be objc.Object; got:\n%s", out)
	}
}
...
func TestEmitProtocolImplsWritesFiles(t *testing.T) {
	...
	framework := &meta.FrameworkMeta{
		...
		Protocols: map[string]meta.Protocol{
			"NSFooDelegate": {
				Methods: []meta.Method{
					{Selector: "didFoo", Return: meta.ReturnType{ObjCType: "void"}},
				},
			},
		},
	}
		...
		Protocols: map[string]meta.Protocol{
			"NSFooDelegate": {
				Methods: []meta.Method{
					...
					{Selector: "shared", IsClassMethod: true, Return: meta.ReturnType{ObjCType: "instancetype"}},
				},
			},
		},
	}
