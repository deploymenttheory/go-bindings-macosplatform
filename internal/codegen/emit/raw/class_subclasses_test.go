...
func TestCollectOverridableMethodsEmpty(t *testing.T) {
	...
	cls := meta.Class{}
	result := collectOverridableMethods(cls, m, map[string]meta.Class{})
	if len(result) != 0 {
...
func TestCollectOverridableMethodsWalksChain(t *testing.T) {
	...
	allClasses := map[string]meta.Class{
		"NSObject": {
			...
			Methods: []meta.Method{
				{Selector: "description", Return: meta.ReturnType{ObjCType: "void"}},
			},
		},
		"NSResponder": {
			Super: "NSObject",
			Methods: []meta.Method{
				{Selector: "mouseDown:", Return: meta.ReturnType{ObjCType: "void"}},
			},
		},
		"NSFoo": {
			Super: "NSResponder",
			Methods: []meta.Method{
				{Selector: "doFoo", Return: meta.ReturnType{ObjCType: "void"}},
			},
		},
	}
...
func TestEmitSubclassFactoriesEmpty(t *testing.T) {
	...
	if err := EmitSubclassFactories(dir, framework, m, map[string]bool{}, map[string]meta.Class{}); err != nil {
		t.Fatal(err)
	}
}
...
func TestEmitSubclassFactoriesWritesFiles(t *testing.T) {
	...
	allClasses := map[string]meta.Class{"NSFoo": framework.Classes["NSFoo"]}
	...
	framework := &meta.FrameworkMeta{
		...
		Classes: map[string]meta.Class{
			"NSFoo": {
				...
				Methods:      []meta.Method{{Selector: "doThing", Return: meta.ReturnType{ObjCType: "void"}}},
			},
		},
	}
	superIndex := map[string]bool{"NSFoo": true}
	if err := EmitSubclassFactories(dir, framework, m, superIndex, map[string]meta.Class{}); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(filepath.Join(dir, "NSFoo_subclass.go")); err == nil {
	...
	framework := &meta.FrameworkMeta{
		...
		Classes: map[string]meta.Class{
			"NSArray": {
				...
				Methods:       []meta.Method{{Selector: "count", Return: meta.ReturnType{ObjCType: "void"}}},
			},
		},
	}
