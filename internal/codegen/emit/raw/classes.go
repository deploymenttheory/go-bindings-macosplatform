...
func EmitClasses(outDir string, framework *meta.FrameworkMeta, m *typemap.Mapper, knownClasses map[string]bool, allClasses map[string]meta.Class, packageName string) error {
...
func EmitClass(w io.Writer, name string, cls meta.Class, framework *meta.FrameworkMeta, m *typemap.Mapper, knownClasses map[string]bool, allClasses map[string]meta.Class, packageName string) error {
	...
	if needsBlocks {
		allImports = append(allImports, "github.com/deploymenttheory/go-bindings-macosplatform/internal/blocks")
	}
...
func classifySuper(className string, cls meta.Class, framework *meta.FrameworkMeta, m *typemap.Mapper) superInfo {
	if cls.Super == "" {
...
func writeStructDef(w io.Writer, name string, cls meta.Class, isGeneric bool, si superInfo) {
	...
	if si.isRoot {
		...
		if isGeneric {
			...
			fmt.Fprintf(w, "var _ objc.Object = (*%s[objc.Object])(nil)\n\n", name)
		} else {
			fmt.Fprintf(w, "func (o *%s) Ptr() unsafe.Pointer { return o.ptr }\n\n", name)
			fmt.Fprintf(w, "var _ objc.Object = (*%s)(nil)\n\n", name)
		}
	} else {
		...
		// objc.Object instantiation when the child is not generic.
		if si.superIsGeneric {
			if isGeneric {
				embedField += "[T]"
			} else {
				embedField += "[objc.Object]"
			}
		}
...
func writeConstructors(w io.Writer, name string, cls meta.Class, isGeneric bool, si superInfo, framework *meta.FrameworkMeta, allClasses map[string]meta.Class, m *typemap.Mapper, knownClasses map[string]bool, usedImports typemap.ImportSet) {
	genSuffix := ""
	if isGeneric {
		genSuffix = "[objc.Object]"
	}
	...
	fmt.Fprintf(w, "\to := %s\n", chain)
	fmt.Fprintf(w, "\tobjc.Track(o, o.Ptr)\n")
	...
	// in other packages that need NSMutableArray[T] (not NSMutableArray[objc.Object]).
	if isGeneric {
		tChain := buildValueChain(name, "[T]", framework.Framework, framework.Classes, m, usedImports)
		tValueChain := strings.TrimPrefix(tChain, "&")
		fmt.Fprintf(w, "func %sTypedWithPtr[T objc.Object](ptr unsafe.Pointer) %s[T] {\n", name, name)
		fmt.Fprintf(w, "\treturn %s\n", tValueChain)
		fmt.Fprintf(w, "}\n\n")
	}
}
...
func writeDesignatedInitConstructors(w io.Writer, name string, cls meta.Class, isGeneric bool, framework *meta.FrameworkMeta, ctx typemap.Context, m *typemap.Mapper, allClasses map[string]meta.Class, imports typemap.ImportSet) {
	if isGeneric {
	...
	for _, method := range cls.Methods {
		...
		// or objc.Object (id post-fix). In the unsafe.Pointer and objc.Object cases we
		...
		initReturnsID := initGoRet == "objc.Object"
		...
		case initReturnsID && method.IsNSError:
			// id (objc.Object) return: extract Ptr() to pass to typed constructor.
...
// Callers should verify the dynamic type with objc.ClassNameOf before relying on it.
func writeCastHelper(w io.Writer, name string, isGeneric bool) {
	...
	fmt.Fprintf(w, "// objc.ClassNameOf before relying on the cast.\n")
...
func classConformsToCoding(cls meta.Class) bool {
...
func writeCodingMethods(w io.Writer, name, framework string, cls meta.Class) {
	if !classConformsToCoding(cls) {
		return
	}
...
//	    return o
//	}
func writeGenericHelper(w io.Writer, name string, cls meta.Class, si superInfo, framework *meta.FrameworkMeta, allClasses map[string]meta.Class, m *typemap.Mapper, usedImports typemap.ImportSet) {
	tChain := buildValueChain(name, "[T]", framework.Framework, framework.Classes, m, usedImports)
	fmt.Fprintf(w, "func New%sT[T objc.Object](ptr unsafe.Pointer) *%s[T] {\n", name, name)
	fmt.Fprintf(w, "\tif ptr == nil {\n\t\treturn nil\n\t}\n")
	fmt.Fprintf(w, "\to := %s\n", tChain)
	fmt.Fprintf(w, "\tobjc.Track(o, o.Ptr)\n")
	fmt.Fprintf(w, "\treturn o\n")
...
func buildValueChain(className, genSuffix, currentFW string, fmClasses map[string]meta.Class, m *typemap.Mapper, usedImports typemap.ImportSet) string {
	return "&" + buildValueChainInner(className, genSuffix, currentFW, fmClasses, m, usedImports)
}

func buildValueChainInner(className, genSuffix, currentFW string, fmClasses map[string]meta.Class, m *typemap.Mapper, usedImports typemap.ImportSet) string {
	...
	if superInFW {
		...
		if len(superCls.GenericParams) > 0 {
			if genSuffix != "" {
				superGenSuffix = genSuffix // generic child → propagate [T] or [objc.Object]
			} else {
				superGenSuffix = "[objc.Object]" // non-generic child of generic super
			}
		}
...
func writeMethod(w io.Writer, em emittedMethod, receiver string, className, framework string, ctx typemap.Context, m *typemap.Mapper, fmClasses map[string]meta.Class, allClasses map[string]meta.Class, pkgTypeNames map[string]bool, imports typemap.ImportSet) error {
	method := em.method
...
func writeInstanceMethod(w io.Writer, goName, receiver string, goArgs []string, goRet string, method meta.Method, cFunc string, ctx typemap.Context, m *typemap.Mapper, fmClasses map[string]meta.Class, imports typemap.ImportSet) {
	retStr := ""
	if goRet != "" {
		retStr = " " + goRet
	}
...
func writeClassMethod(w io.Writer, goName, className string, goArgs []string, goRet string, method meta.Method, cFunc string, ctx typemap.Context, m *typemap.Mapper, fmClasses map[string]meta.Class, pkgTypeNames map[string]bool, imports typemap.ImportSet) {
...
func writeMethodBody(w io.Writer, method meta.Method, cFunc string, isClassMethod bool, ctx typemap.Context, m *typemap.Mapper, fmClasses map[string]meta.Class, imports typemap.ImportSet) {
	...
	if retType == "objc.Object" || isObjectReturn(retType) || isNullableStr || isValueStruct {
	...
	if method.IsNSError {
		if retType == "" {
		...
		} else if retType == "objc.Object" {
			fmt.Fprintf(w, "\tif _nsErr != nil {\n\t\treturn nil, tel.NSErrorToError(ctx, _nsErr)\n\t}\n")
			fmt.Fprintf(w, "\treturn objc.WrapObject(_ptr), nil\n")
			...
			fmt.Fprintf(w, "\tobjc.FreePtr(_ptr)\n")
	...
	if retType == "objc.Object" {
		fmt.Fprintf(w, "\treturn objc.WrapObject(_ptr)\n")
		return
	}
...
// extractStructType strips the leading "*" and substitutes [objc.Object] for [T]
...
func extractStructType(retType string, isClassMethod bool) string {
	...
	if isClassMethod {
		s = strings.ReplaceAll(s, "[T]", "[objc.Object]")
	}
	return s
}

// writeObjectReturn emits an object construction return using objc.WrapTyped.
...
//
// Three cases:
//  1. Cross-framework type ("foundation.NSString"): objc.WrapTyped(_ptr, pkg.NewTypeName)
//  2. Same-framework T-generic ("NSArray[T]"): objc.WrapTyped(_ptr, NewNSArrayT[T])
//  3. Same-framework non-T ("NSString", "NSArray[objc.Object]"): objc.WrapTyped(_ptr, NewNSString)
func writeObjectReturn(w io.Writer, structType string, fmClasses map[string]meta.Class, m *typemap.Mapper) {
	fmt.Fprintf(w, "\treturn objc.WrapTyped(_ptr, %s)\n", constructorRef(structType, fmClasses))
}

// writeObjectReturnWithError emits the object + error return using objc.WrapTyped.
func writeObjectReturnWithError(w io.Writer, structType string, fmClasses map[string]meta.Class, m *typemap.Mapper) {
	fmt.Fprintf(w, "\treturn objc.WrapTyped(_ptr, %s), nil\n", constructorRef(structType, fmClasses))
}
...
func objectConstructExpr(structType, ptrVar string, fmClasses map[string]meta.Class, m *typemap.Mapper) string {
...
// objc.WrapTyped. Unlike objectConstructExpr, it returns the function itself
...
func constructorRef(structType string, fmClasses map[string]meta.Class) string {
	if isCrossFrameworkType(structType) {
		dot := strings.Index(structType, ".")
		pkg := structType[:dot]
		typeName := structType[dot+1:]
...
func blockNeedsWrapper(blockObjCType string, ctx typemap.Context, m *typemap.Mapper) bool {
	...
	for _, a := range args {
		...
		// Bare `id` needs wrapping: MakeBlock_* expects unsafe.Pointer but user sees objc.Object.
		if typemap.IsID(a) {
			return true
		}
		...
		// needs wrapping unsafe.Pointer → objc.Object via objc.WrapObject.
...
//   - error (NSError *): objc.NSErrorToError(pName)
...
func blockArgCtor(goType, pName string) string {
	...
	if strings.HasPrefix(goType, "*") {
		...
		// Without this, strings.Index finds the "." inside "[objc.Object]" and
		...
		// *Foo[objc.Object] but the user func may expect *Foo[T].
		if hasTypeParam {
		...
		if dot := strings.Index(baseName, "."); dot >= 0 {
			pkg := baseName[:dot]
			name := baseName[dot+1:]
			return "objc.WrapTyped(" + pName + ", " + pkg + ".New" + name + ")"
		}
		return "objc.WrapTyped(" + pName + ", New" + baseName + ")"
	}
	return pName
}
...
// NSError * args are converted via objc.NSErrorToError; known ObjC class pointer args
...
func buildBlockWrapper(blockObjCType, userArgName string, ctx typemap.Context, m *typemap.Mapper, imports typemap.ImportSet) string {
	...
	for _, a := range args {
		...
		if typemap.IsNSError(a) {
			...
			n := strings.TrimSpace(typemap.Normalise(a))
			// Bare id (any ObjC object) → objc.Object.
			if typemap.IsID(n) {
				goType = "objc.Object"
				...
				// or objc.Object — both are constructible from unsafe.Pointer by blockArgCtor.
				retCtx := argCtx
				...
				// blockArgCtor handles "objc.Object" via objc.WrapObject.
	...
	if retIsID || retIsIDProtocol || (typemap.ClassName(retType) != "" && ctx.ClassNameIndex[typemap.ClassName(retType)]) {
		...
		if typed != "" && typed != "unsafe.Pointer" {
			...
			if typed == "objc.Object" {
				// objc.Object return: convert via Ptr() (may be nil).
				if hasBoolPtr {
...
func methodHasBlockArgs(args []meta.Param, m *typemap.Mapper) bool {
...
func buildCGOCallArgs(args []meta.Param, isClassMethod, hasNSError, withException bool, ctx typemap.Context, m *typemap.Mapper, preambles, keepAlives *[]string, imports typemap.ImportSet) string {
	var parts []string
...
// instead of emitting individual defer objc.KeepAlive statements.
...
func primaryReturnType(method meta.Method, ctx typemap.Context, m *typemap.Mapper, imports typemap.ImportSet) string {
	ret := method.Return
	if ret.IsGeneric && len(ctx.GenericParams) > 0 {
		// Generic element return (e.g. firstObject → ObjectType *).
		// Matches buildGoReturn: both emit objc.Object so the body is consistent.
		return "objc.Object"
	}
	if ret.IsInstancetype || typemap.IsInstancetype(ret.ObjCType) {
		if ctx.ClassName != "" {
			...
			if len(ctx.GenericParams) > 0 {
				...
				retType = "*" + ctx.ClassName + "[objc.Object]"
			}
			return retType
		}
...
func nsStringArgIndices(args []meta.Param) []int {
	var idxs []int
	for i, arg := range args {
...
func nsStringOverloadArgs(args []meta.Param, hasNSError bool, nsIdxs []int, ctx typemap.Context, m *typemap.Mapper, imports typemap.ImportSet) []string {
...
func nsStringConvertArg(argName string, isNS bool, ctx typemap.Context, m *typemap.Mapper, imports typemap.ImportSet) string {
	...
	if owner == "" || strings.EqualFold(owner, ctx.Framework) {
		return "NewNSString(objc.GoStringToNSString(" + argName + "))"
	}
	pkg := strings.ToLower(owner)
	...
	return pkg + ".NewNSString(objc.GoStringToNSString(" + argName + "))"
}
...
func writeNSStringInstanceOverload(w io.Writer, goName, receiver string, method meta.Method, ctx typemap.Context, m *typemap.Mapper, imports typemap.ImportSet) {
...
func writeNSStringClassOverload(w io.Writer, goName, className string, method meta.Method, ctx typemap.Context, m *typemap.Mapper, pkgTypeNames map[string]bool, imports typemap.ImportSet) {
	nsIdxs := nsStringArgIndices(method.Params)
	if len(nsIdxs) == 0 {
		return
	}
