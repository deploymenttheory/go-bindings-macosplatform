...
//   - Embeds foundation.NSObject, satisfying objc.Object and NSObjectProtocol
//   - Exports a New<Name> constructor that registers a GC finalizer via objc.Track
...
func EmitProtocolProxies(w io.Writer, pkgName, packageName string, framework *meta.FrameworkMeta, m *typemap.Mapper, knownClasses map[string]bool, allClasses map[string]meta.Class) error {
	...
	for _, name := range names {
		...
		model, hasCtx, hasBlks := buildProtocolIDTypeModel(name, p, framework, m, allClasses, ctx, usedImports)
...
func buildProtocolIDTypeModel(name string, p meta.Protocol, framework *meta.FrameworkMeta, m *typemap.Mapper, allClasses map[string]meta.Class, ctx typemap.Context, imports typemap.ImportSet) (model protocolProxyTypeModel, needsContext, needsBlocks bool) {
	...
	for _, method := range p.Methods {
		...
		args := append([]string{"ctx context.Context"}, buildGoArgs(method.Params, method.IsNSError, idCtx, m, imports)...)
	...
	return model, needsContext, needsBlocks
}
...
func writeIDProtocolMethodBody(w io.Writer, method meta.Method, cFunc string, framework, idTypeName string, ctx typemap.Context, m *typemap.Mapper, fmClasses map[string]meta.Class, imports typemap.ImportSet) {
	...
	spanName := fmt.Sprintf("%s.%s/%s", framework, idTypeName, method.Selector)
	telExtra := buildTelCallExtra(keepAlives)
	fmt.Fprintf(w, "\tctx, _end := tel.Call(ctx, p, %q%s)\n", spanName, telExtra)
...
func buildIDProtocolCGOCallArgs(args []meta.Param, hasNSError bool, ctx typemap.Context, m *typemap.Mapper, preambles, keepAlives *[]string, imports typemap.ImportSet) string {
	var parts []string
