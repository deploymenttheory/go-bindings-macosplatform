...
func TestBuildGoReturnVoid(t *testing.T) {
	...
	method := meta.Method{
		...
		Return:   meta.ReturnType{ObjCType: ""},
	}
...
func TestBuildGoReturnPrimitive(t *testing.T) {
	...
	method := meta.Method{
		Selector: "count",
		Return:   meta.ReturnType{ObjCType: "uint64_t"},
	}
	got := buildGoReturn(method, ctx, m, "", nil)
	if got != "uint64" {
...
func TestAvailabilityCommentPresent(t *testing.T) {
	av := meta.Availability{MacOSIntroduced: "12.0"}
...
func TestIsValueStructReturnPrimitive(t *testing.T) {
	for _, typ := range []string{"bool", "int32", "uint64", "float32", "string", "error", "objc.Object"} {
