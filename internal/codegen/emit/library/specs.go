...
func EmitSpecs(w io.Writer, pkgName, rawImportPath string, framework *meta.FrameworkMeta, m *typemap.Mapper, knownClasses map[string]bool) error {
	...
	for _, spec := range specs {
		for _, f := range spec.fields {
			...
			if strings.Contains(f.goType, "objc.") {
				needsObjc = true
			}
		}
	}
	...
	for _, spec := range specs {
		...
		for _, f := range spec.fields {
			if f.isSlice {
				...
				fmt.Fprintf(&body, "\t\t_objs := make([]objc.Object, len(spec.%s))\n", f.name)
				fmt.Fprintf(&body, "\t\tfor _i, _d := range spec.%s { _objs[_i] = _d }\n", f.name)
				fmt.Fprintf(&body, "\t\tc.%s(ctx, foundation.NSArrayOf[objc.Object](ctx, _objs...))\n", f.setter)
				fmt.Fprintf(&body, "\t}\n")
