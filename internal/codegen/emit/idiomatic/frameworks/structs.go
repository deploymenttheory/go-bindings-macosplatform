...
func emitStructs(
	...
	for _, goName := range goNames {
		...
		if mapper.ByteArrayStructs[goName] && s.Size > 0 {
			...
			layoutChecks = append(layoutChecks, layoutCheck{goName, s.Size, s.Align})
			continue
		}
		...
		if s.Size != 0 {
			layoutChecks = append(layoutChecks, layoutCheck{goName, s.Size, 0})
		}
	}
	...
	return writeLayoutTest(outDir, pkgName, layoutChecks)
}
...
// writeLayoutTest emits <pkg>_layout_generated_test.go: a reflect-based test that
// asserts every emitted value/byte-array struct reproduces the authoritative C
// ABI size (and alignment, capped at Go's max 8) clang reported. It is the ABI
// regression gate the name-keyed parity check cannot provide — a layout bug (e.g.
// a packed or union struct emitted at the wrong size) fails `go test` here.
func writeLayoutTest(outDir, pkgName string, checks []layoutCheck) error {
	if len(checks) == 0 {
		return nil
	}
	...
	b.WriteString("\t}\n}\n")
	return rawfw.WriteGoFile(filepath.Join(outDir, pkgName+"_layout_generated_test.go"), []byte(b.String()))
}
