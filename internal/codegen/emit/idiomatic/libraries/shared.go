...
func writeOpinionatedHeader(
	...
	imps := make([]view.ImportLineModel, 0, len(all))
	for alias, path := range all {
		imps = append(imps, view.ImportLineModel{
			Alias: alias,
			Path:  path,
			Bare:  strings.HasSuffix(path, "/"+alias) || alias == path,
		})
	}
	sort.Slice(imps, func(i, j int) bool { return imps[i].Path < imps[j].Path })

	return render.Execute(w, "opinionated_header", view.OpinionatedHeaderModel{
		PkgName: pkgName,
		Imports: imps,
	})
}
