...
func methods(sels ...string) []meta.Method {
	out := make([]meta.Method, len(sels))
	for i, s := range sels {
		out[i].Selector = s
	}
	return out
...
func countMap(ms []meta.Method) map[string]int {
...
func TestRule2_IBActionSkipped(t *testing.T) {
	goBack := meta.Method{Selector: "goBack"}
	goBackSender := meta.Method{
		Selector: "goBack:",
		Params:     []meta.Param{{ObjCType: "id"}},
	}
	ms := []meta.Method{goBack, goBackSender}
...
func TestRule3_GenuineCollision(t *testing.T) {
	m1 := meta.Method{Selector: "transformContextForBox:"}
	m2 := meta.Method{Selector: "transformContext:forBox:"}
	ms := []meta.Method{m1, m2}
...
func TestUniqueNamePassthrough(t *testing.T) {
	m := meta.Method{Selector: "doSomething"}
	ms := []meta.Method{m}
...
func TestClassMethodPassthrough(t *testing.T) {
	m := meta.Method{Selector: "sharedManager", IsClassMethod: true}
	sibling := meta.Method{Selector: "sharedManager:", IsClassMethod: true}
	ms := []meta.Method{m, sibling}
...
func TestFindSiblingMethod(t *testing.T) {
	inst := meta.Method{Selector: "open", IsClassMethod: false}
	cls := meta.Method{Selector: "open", IsClassMethod: true}
	ms := []meta.Method{inst, cls}
