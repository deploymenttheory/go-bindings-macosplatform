...
// directory path that was written to.  The file is named per meta.Write
// convention: <Framework>-<Arch>-<SDKVersion>.gometa.json.
func writeMeta(t *testing.T, dir string, framework *meta.FrameworkMeta) string {
	t.Helper()
	if err := meta.Write(framework, dir); err != nil {
		t.Fatalf("meta.Write: %v", err)
	}
	return dir
}

// minimalFM returns a FrameworkMeta with sane defaults so meta.Write succeeds.
func minimalFM(framework string) *meta.FrameworkMeta {
...
func TestLoadAllKnownClasses(t *testing.T) {
	...
	framework.Classes["NSString"] = meta.Class{
		Methods: []meta.Method{{Selector: "length", Return: meta.ReturnType{ObjCType: "NSUInteger"}}},
	}
...
func TestLoadAllGenericClasses(t *testing.T) {
	dir := t.TempDir()
	framework := minimalFM("Foundation")
	framework.Classes["NSArray"] = meta.Class{
		GenericParams: []string{"ObjectType"},
		Methods:       []meta.Method{{Selector: "count", Return: meta.ReturnType{ObjCType: "NSUInteger"}}},
	}
	writeMeta(t, filepath.Join(dir, "Foundation"), framework)
...
func TestLoadAllFrameworkOwnerFewestMethods(t *testing.T) {
	...
	fmFoundation.Classes["NSString"] = meta.Class{
		Methods: []meta.Method{{Selector: "length", Return: meta.ReturnType{ObjCType: "NSUInteger"}}},
	}
	...
	fmUIKit.Classes["NSString"] = meta.Class{
		Methods: []meta.Method{
...
func TestLoadAllGhostsFallBack(t *testing.T) {
	...
	fmA := minimalFM("Alpha")
	fmA.Classes["NSGhost"] = meta.Class{} // score 0
	writeMeta(t, filepath.Join(dir, "Alpha"), fmA)

	fmB := minimalFM("Beta")
	fmB.Classes["NSGhost"] = meta.Class{} // score 0
...
func TestLoadAllTopLevelBeatsSubFramework(t *testing.T) {
	...
	topFM.Classes["CGPath"] = meta.Class{
		Methods: []meta.Method{{Selector: "isRect", Return: meta.ReturnType{ObjCType: "BOOL"}}},
	}
	...
	subFM.Classes["CGPath"] = meta.Class{} // ghost
...
func TestLoadAllParentFrameworkInferred(t *testing.T) {
	...
	hiMeta.Classes["HIView"] = meta.Class{
		Methods: []meta.Method{{Selector: "getID", Return: meta.ReturnType{ObjCType: "uint32"}}},
	}
...
func TestLoadAllAllClassesPopulated(t *testing.T) {
	dir := t.TempDir()
	framework := minimalFM("Foundation")
	framework.Classes["NSObject"] = meta.Class{
		Methods: []meta.Method{{Selector: "init", Return: meta.ReturnType{IsInstancetype: true}}},
	}
	writeMeta(t, filepath.Join(dir, "Foundation"), framework)
...
func TestLoadAllDirectFilePath(t *testing.T) {
	...
	if len(entries) == 0 {
		t.Fatal("no files written by meta.Write")
	}
...
func TestLoadAllNonGhostBeatsGhost(t *testing.T) {
	...
	fmA := minimalFM("Alpha")
	fmA.Classes["NSWidget"] = meta.Class{} // ghost
	writeMeta(t, filepath.Join(dir, "Alpha"), fmA)

	// "Beta" has a real entry for NSWidget (score 1).
	fmB := minimalFM("Beta")
	fmB.Classes["NSWidget"] = meta.Class{
		Methods: []meta.Method{{Selector: "draw", Return: meta.ReturnType{ObjCType: "void"}}},
	}
	writeMeta(t, filepath.Join(dir, "Beta"), fmB)
