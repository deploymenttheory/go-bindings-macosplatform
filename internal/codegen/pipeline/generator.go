...
import (
	...
	"github.com/deploymenttheory/go-bindings-macosplatform/internal/codegen/emit/ergonomic"
	"github.com/deploymenttheory/go-bindings-macosplatform/internal/codegen/emit/library"
	"github.com/deploymenttheory/go-bindings-macosplatform/internal/codegen/emit/raw"
...
// the shared block and callback trampoline files in internal/blocks and internal/callbacks.
type BindingsConfig struct {
	...
	SDKPath string
	// BlocksDir overrides the output path for internal/blocks generated files.
	...
	BlocksDir string
	// CallbacksDir overrides the output path for internal/callbacks generated files.
...
const (
	defaultBlocksDir    = "internal/blocks"
	defaultCallbacksDir = "internal/callbacks"
)
...
// ErgonomicConfig controls generation of the opinionated/ergonomic layer.
type ErgonomicConfig struct {
	// Registry is the combined metadata for all frameworks.
	Registry *Registry
	// OutDir is the root output directory for the ergonomic layer.
	// Canonical value: <repo-root>/opinionated/ergonomic
	OutDir string
...
func GenerateBindings(cfg BindingsConfig) error {
	...
	// Block trampolines.
	{
		frameworks := make([]*meta.FrameworkMeta, 0, len(reg.Frameworks))
	...
	if err := forEachFramework(reg, nil, func(framework *meta.FrameworkMeta) error {
...
func GenerateCustom(cfg CustomConfig) error {
	...
	return forEachFramework(reg, cfg.Frameworks, func(framework *meta.FrameworkMeta) error {
...
// GenerateErgonomic writes the opinionated/ergonomic layer.
// For a full regen (no framework filter) each framework's output directory is
// removed individually before regeneration, preserving sibling directories such
// as docs/ and shared/ that are not owned by the generator.
func GenerateErgonomic(cfg ErgonomicConfig) error {
	reg := cfg.Registry
	m := buildMapper(reg, false)
	...
	if len(cfg.Frameworks) == 0 {
		for _, framework := range reg.Frameworks {
			...
			if err := os.RemoveAll(filepath.Join(cfg.OutDir, subdir, packageName)); err != nil {
				return fmt.Errorf("clean ergonomic %s: %w", packageName, err)
			}
		}
	}

	return forEachFramework(reg, cfg.Frameworks, func(framework *meta.FrameworkMeta) error {
		if err := emitErgonomic(cfg, framework, m, reg); err != nil {
			return fmt.Errorf("ergonomic %s: %w", framework.Framework, err)
		}
		return nil
	})
}

// emitErgonomic writes *_generated.go files for the ergonomic layer for one framework.
// Only files matching *_generated.go are removed on each run; hand-crafted files are never
// touched. Swift-only, umbrella, and unsupported-bridge frameworks are skipped silently.
func emitErgonomic(
	cfg ErgonomicConfig,
	...
	outDir := filepath.Join(cfg.OutDir, subdir, packageName)
	...
	nt := ergonomic.NewNameTracker()

	if err := writeFile(
		filepath.Join(outDir, packageName+"_constructors_generated.go"),
		func(buf *bytes.Buffer) error {
			return ergonomic.EmitConstructors(
				buf,
				packageName,
				rawImportPath,
				framework,
	...
	if err := writeFile(
		filepath.Join(outDir, packageName+"_async_generated.go"),
		func(buf *bytes.Buffer) error {
			return ergonomic.EmitAsync(
				buf,
				packageName,
				rawImportPath,
				framework,
	...
	if err := writeFile(
		filepath.Join(outDir, packageName+"_collections_generated.go"),
		func(buf *bytes.Buffer) error {
			return ergonomic.EmitCollections(
				buf,
				packageName,
				rawImportPath,
				framework,
	...
	if err := writeFile(
		filepath.Join(outDir, packageName+"_properties_generated.go"),
		func(buf *bytes.Buffer) error {
			return ergonomic.EmitProperties(
				buf,
				packageName,
				rawImportPath,
				framework,
	...
	if err := writeFile(
		filepath.Join(outDir, packageName+"_block_enum_generated.go"),
		func(buf *bytes.Buffer) error {
			return ergonomic.EmitBlockEnum(
				buf,
				packageName,
				rawImportPath,
				framework,
	...
	if err := writeFile(
		filepath.Join(outDir, packageName+"_main_thread_generated.go"),
		func(buf *bytes.Buffer) error {
			return ergonomic.EmitMainThread(
				buf,
				packageName,
				rawImportPath,
				framework,
	...
	if err := writeFile(
		filepath.Join(outDir, packageName+"_delegates_generated.go"),
		func(buf *bytes.Buffer) error {
			return ergonomic.EmitDelegates(
				buf,
				packageName,
				rawImportPath,
				framework,
	...
	if err := writeFile(
		filepath.Join(outDir, packageName+"_aliases_generated.go"),
		func(buf *bytes.Buffer) error {
			return ergonomic.EmitTypeAliases(
				buf,
				packageName,
				rawImportPath,
				framework,
...
func forEachFramework(reg *Registry, filter []string, fn func(*meta.FrameworkMeta) error) error {
...
func sortFrameworksByDependency(reg *Registry) []*meta.FrameworkMeta {
	// Build dependency edges: framework → set of frameworks it depends on.
	deps := make(map[string]map[string]bool)
	byName := make(map[string]*meta.FrameworkMeta)
	for _, framework := range reg.Frameworks {
	...
	if len(framework.Structs) > 0 || len(framework.Typedefs) > 0 {
		if err := writeFile(
			...
			func(buf *bytes.Buffer) error {
				...
				needsObjc := bytes.Contains(body.Bytes(), []byte("objc.Track"))
...
// writeGoHeaderStructs writes a file header for structs files that need
// both unsafe and the objc package (e.g. when CF opaque wrappers are emitted).
// Deprecated: use writeGoHeaderStructsFull.
func writeGoHeaderStructs(buf *bytes.Buffer, pkgName string) {
	writeGoHeaderStructsFull(buf, pkgName, true, true, nil)
}
...
func writeGoHeaderRuntime(buf *bytes.Buffer, pkgName string) {
	...
	fmt.Fprintf(buf, "\t\"github.com/deploymenttheory/go-bindings-macosplatform/internal/objc\"\n")
	fmt.Fprintf(buf, ")\n\n")
	fmt.Fprintf(buf, "var _ unsafe.Pointer   // suppress unused import\n")
	fmt.Fprintf(buf, "var _ objc.Object = nil // suppress unused import\n\n")
}
...
func hasBridgeableFunctions(framework *meta.FrameworkMeta) bool {
	for _, fn := range framework.Functions {
		if fn.IsInline {
			continue
		}
