...
func (m *Mapper) CType(qt string, ctx Context, imports ImportSet) string {
	...
	// NSObject<OS_dispatch_queue>) both surface as "objc.Object" from GoType.
...
func (m *Mapper) resolveGoType(n string, ctx Context, imports ImportSet) string {
	...
	// import-blocked), fall back to objc.Object (the common ObjC object interface).
	if protos := IDProtocols(n); len(protos) > 0 {
		...
		// Unknown protocols (none in ProtocolIndex): use objc.Object as the
		// minimum guarantee that the caller passes a valid ObjC object wrapper.
		return "objc.Object"
	}

	// id → objc.Object. Bare `id` is an untyped ObjC object reference; objc.Object
	...
	// Return objc.Object — same as buildGoReturn does when IsGeneric=true — since T cannot
		...
		if baseClass != "" && ctx.ClassNameIndex[baseClass] {
			...
			if anyIsTypeVar {
				if ctx.IsClassMethod {
					// Class methods have no [T] in scope; use objc.Object.
					return m.qualifiedType(baseClass, baseClass+"[objc.Object]", ctx, imports)
				}
	...
	if cls != "" {
		...
		if ctx.ClassNameIndex[cls] {
			if m.GenericClasses[cls] {
				return m.qualifiedType(cls, cls+"[objc.Object]", ctx, imports)
			}
		...
		if target, ok := m.TypedefIndex[cls]; ok && target != cls {
			...
			for range 8 {
				...
				if resolvedCls != "" {
					...
					// (e.g. MPSShape → NSArray<NSNumber *> → *foundation.NSArray[objc.Object]).
