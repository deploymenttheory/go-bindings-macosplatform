...
func (m *Mapper) GoBlockUserFuncType(n string, ctx Context, imports ImportSet) string {
	...
	for _, a := range args {
		...
		if IsNSError(a) {
			...
			// bare `id` in a user-facing block arg → objc.Object interface
			t = "objc.Object"
		} else if protos := IDProtocols(a); len(protos) > 0 {
			// id<Protocol> block arg: use return-position semantics so GoType yields
			// *ProtoIDProtocol (constructible via NewProto(ptr)) or objc.Object —
			...
			if _, ok := m.CFTypeIndex[n]; ok {
				...
				// Return objc.Object — blockArgCtor converts via objc.WrapObject.
