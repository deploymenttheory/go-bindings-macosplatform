...
func TestResolveGoTypeTypedefToUnsafeDoesntShortCircuit(t *testing.T) {
	m := newTestMapper()
	m.TypedefIndex = map[string]string{"OpaqueHandle": "id"} // id → objc.Object
	// A typedef that resolves to id propagates the objc.Object result upward.
	ctx := newTestCtx(m, "Foundation")
	got := m.GoType("OpaqueHandle", ctx, nil)
	// id → objc.Object, so the typedef resolves to objc.Object.
	if got != "objc.Object" {
		t.Errorf("GoType(typedef→id) = %q, want objc.Object", got)
	}
}
...
func TestGoPointerTypeInClassMethod(t *testing.T) {
	...
	// → qualifiedType with "[objc.Object]" instead of "[T]"
	got := m.GoType("NSArray<ObjectType> *", ctx, nil)
	if got != "*NSArray[objc.Object]" {
		t.Errorf("GoType(NSArray<ObjectType> *, InClassMethod) = %q, want *NSArray[objc.Object]", got)
	}
}
...
func TestResolveGoTypeIDProtocolReturnPosition(t *testing.T) {
	...
	ctx := newTestCtx(m, "Foundation")
	ctx.IsReturn = true
	// In return position with no proxy registered, id<Protocol> falls back to objc.Object.
	got := m.GoType("id<NSCopying>", ctx, nil)
	if got != "objc.Object" {
		t.Errorf("GoType(id<NSCopying>, IsReturn) = %q, want objc.Object", got)
	}
}
