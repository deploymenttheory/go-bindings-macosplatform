...
const (
	PureobjcImport = "github.com/deploymenttheory/go-bindings-macosplatform/internal/pureobjc"
	PureobjcPkg    = "pureobjc"
...
func emitClass(
	...
	// Always include objc and pureobjc.
...
func writeFromIDConstructor(w io.Writer, className string, isGeneric bool, genericParams []string) {
	...
	fmt.Fprintf(w, "\tpureobjc.Track(o)\n")
...
func writeMethodSendCall(
	...
	case call.retIsVoid:
		...
		if method.IsNSError {
			...
			fmt.Fprintf(w, "\t\treturn pureobjc.NSErrorToError(objc.ID(_nsErr))\n")
			fmt.Fprintf(w, "\t}\n")
	...
	case call.retIsObject:
		...
		if method.IsNSError {
			fmt.Fprintf(w, "\tif _nsErr != 0 {\n")
			fmt.Fprintf(w, "\t\treturn nil, pureobjc.NSErrorToError(objc.ID(_nsErr))\n")
			fmt.Fprintf(w, "\t}\n")
	...
	case call.retGoType == "string":
		...
		if method.IsNSError {
			...
			fmt.Fprintf(w, "\t\treturn \"\", pureobjc.NSErrorToError(objc.ID(_nsErr))\n")
			fmt.Fprintf(w, "\t}\n")
			fmt.Fprintf(w, "\treturn pureobjc.GoString(_ret), nil\n")
		} else {
			fmt.Fprintf(w, "\treturn pureobjc.GoString(_ret)\n")
		}
	...
	case call.retGoType == "bool":
		...
		if method.IsNSError {
			fmt.Fprintf(w, "\tif _nsErr != 0 {\n")
			fmt.Fprintf(w, "\t\treturn false, pureobjc.NSErrorToError(objc.ID(_nsErr))\n")
			fmt.Fprintf(w, "\t}\n")
	...
	default:
		...
		if method.IsNSError {
			...
			fmt.Fprintf(w, "\tif _nsErr != 0 {\n")
			fmt.Fprintf(w, "\t\treturn %s, pureobjc.NSErrorToError(objc.ID(_nsErr))\n", zeroVal)
			fmt.Fprintf(w, "\t}\n")
			fmt.Fprintf(w, "\treturn _ret, nil\n")
		} else {
			fmt.Fprintf(w, "\treturn _ret\n")
