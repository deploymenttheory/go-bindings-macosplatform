...
All AppKit (and generally all UI-related) calls **must run on the macOS main thread**. Failure to do so causes undefined behaviour and crashes. The generated bindings do not dispatch automatically — the caller is responsible.

The standard pattern for a UI app is to lock the main goroutine to the main OS thread, hand it to the AppKit run loop, and dispatch UI work from other goroutines via the `mainthread` helper package (pure Go, GCD main queue under the hood):
